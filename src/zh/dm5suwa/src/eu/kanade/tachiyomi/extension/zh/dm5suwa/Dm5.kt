...
class Dm5 : ParsedHttpSource(), ConfigurableSource {
    ...
    override fun mangaDetailsParse(document: Document) = SManga.create().apply {
        ...
        status = when (document.selectFirst("div.banner_detail_form p.tip > span > span")!!.text()) {
            "ËøûËΩΩ‰∏? -> SManga.ONGOING
            "Â∑≤Â?Áª? -> SManga.COMPLETED
            else -> SManga.UNKNOWN
        }
    }
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        val container = document.selectFirst("div#chapterlistload")
            ?: throw Exception("ËØ∑Âà∞ WebView Á°ÆËÆ§ÔºõÂ??¢Á?ÁªúÁéØÂ¢ÉÂ??ØÂ?ËØïÊâ©Â±ïËÆæÁΩÆÈ??¢Á??úÔ??®Êº´Â±ã‰??®Ô?Ê∏ÖÈô§ Cookie??)
        ...
        return if (document.selectFirst("div.detail-list-title a.order")!!.text() == "Ê≠??") {
            li.reversed()
        } else {
            li
        }
